...
int isEmpty(QueueType *cQ) {
	...
	if (first == end) {		//아무것도 들어있지 않다면 front와 rear가 같아지니 
	//만약 둘이 같다면 아무것도 없다는 의미이다
...
int isFull(QueueType *cQ) {
	...
	if (first == end) {							//+1을 해서 빈 곳을 임의로 채운다
		...
		return 0;		//다 차있다면 0 출력
	}else{
		return 1;		//다 차있지 않다면 1 출력
	}
}
...
void enQueue(QueueType *cQ, element item) {
	if(!isFull(cQ)) {		//꽉 차있다면 0을 출력받고 !로 인해 1로 바뀜
		...
		//1을 더했을 때 최대크기가 넘어갔을 때 오류가 생기지 않게 하기 위해서 나눠줌
...
/* complete the function */
void deQueue(QueueType *cQ) {		//큐에서 front 앞의 값을 지울 때 item이란 변수는 필요 없기 때문에 제거
